...
from modeling_ropes import RobertaForQuestionAnsweringMultiAnswer
...
from transformers.data.processors.squad import RopesProcessor, ropes_convert_examples_to_features
...
MODEL_CLASSES = {
    ...
    "roberta": (RobertaConfig, RobertaForQuestionAnsweringMultiAnswer, RobertaTokenizer),
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "all_multi_start_position":batch[7],
                "all_multi_end_position":batch[8],
            }
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if args.metadata_dir:
        ...
        output_nbest_file = os.path.join(args.metadata_dir, "nbest_predictions_{}.json".format(prefix))
        output_rewrite_file = os.path.join(args.metadata_dir, "rewrite_question.json")
    else:
        output_prediction_file = os.path.join(args.output_dir, "predictions_{}.json".format(prefix))
        output_nbest_file = os.path.join(args.output_dir, "nbest_predictions_{}.json".format(prefix))
        output_rewrite_file = os.path.join(args.output_dir, "rewrite_question.json")
    ...
    if args.model_type in ["xlnet", "xlm"]:
        ...
        predictions = compute_predictions_logits_include_question(
            examples,
            features,
            all_results,
            args.n_best_size,
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            if evaluate:
                examples = processor.get_dev_examples(args.data_dir, filename=args.predict_file,grounding_type=args.grounding_type,multi_answer=args.multi_answer)
            else:
                examples = processor.get_train_examples(args.data_dir, filename=args.train_file,grounding_type=args.grounding_type,multi_answer=args.multi_answer)
...
def main():
    ...
    if args.metadata_dir and not os.path.exists(args.metadata_dir):
